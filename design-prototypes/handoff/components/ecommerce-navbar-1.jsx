/**
 * EcommerceNavbar1 — Port of shadcn-blocks ecommerce-navbar-1 with ALL features:
 *   - Desktop NavigationMenu with mega-menu dropdowns (sections + featured items)
 *   - Per-item accent color (e.g. "Sale" in red)
 *   - Country/currency selector
 *   - Secondary nav: user, shopping bag with badge, search
 *   - Responsive: mobile Sheet drawer with drill-down (MenuItem → MegaMenu → SubMenu)
 *   - Helpful links, contact info, social links in mobile drawer footer area
 *   - Logo + image + text
 *
 * Real location: src/components/shadcnblocks/ecommerce-navbar-1.tsx
 */
...
const DesktopNav = ({ menu, home }) => {
  const [openId, setOpenId] = React.useState(null); // id of currently-open mega menu
  ...
  return (
    <nav
      className="relative w-full max-w-full bg-background"
    ...
    >
      ...
      {/* Mega menu panel — single slot, content swaps based on openId */}
      {openId && (() => {
...
const DesktopMenuItem = ({ item, isOpen, onEnter }) => {
  const { ChevronDown } = window;
  const style = { color: item.accentColor };
  const base = "inline-flex h-9 items-center whitespace-nowrap rounded-md px-3 text-xs font-semibold uppercase tracking-wide transition hover:bg-muted focus:outline-none";

  const badge = item.badge ? (
    <span className="ml-1.5 inline-flex h-4 items-center rounded-full bg-[oklch(58.6%_0.253_17.585)] px-1.5 text-[9px] font-bold text-white">{item.badge}</span>
  ) : null;

  if (!item.megaMenu) {
    return <a href={item.href} style={style} className={base} onMouseEnter={onEnter}>{item.label}{badge}</a>;
  }
  return (
    <button
      type="button"
...
const MenuLinksList = ({ lists }) => (
  <ul className="flex gap-[1.875rem]">
    {lists.map((list, i) => <li key={i} className="basis-1/3"><MenuLinks list={list}/></li>)}
  </ul>
);

const MenuLinks = ({ list }) => (
  <div className="space-y-1.5">
    <h2 className="px-1 text-xs font-semibold uppercase leading-relaxed tracking-wider"><a href={list.href}>{list.label}</a></h2>
    <ul>
      {list.items.map((item, i) => (
        <li key={i}>
          <a href={item.href} className="block w-full rounded-sm px-1 py-0.5 text-xs leading-normal text-muted-foreground hover:bg-muted">{item.label}</a>
        </li>
      ))}
    </ul>
  </div>
...
const MenuFeaturedLinks = ({ lists }) => (
  <div className="flex gap-[1.875rem]">
    {lists.map((it, i) => <div className="flex-1" key={i}><MenuFeaturedLink {...it}/></div>)}
  </div>
);

const MenuFeaturedLink = ({ href, imageSrc, label }) => (
  <a href={href} className="group/feat block space-y-3">
    <div className="relative aspect-square overflow-hidden">
      <img src={imageSrc} alt={label} className="absolute inset-0 size-full object-cover object-center transition-all duration-500 group-hover/feat:scale-105 group-hover/feat:opacity-80"/>
    </div>
...
const SecondaryNav = () => {
  const { CircleUserRound, ShoppingBag, Search } = window;
  return (
    <div className="flex items-center gap-1">
      <IconButton title="Account"><CircleUserRound size={18}/></IconButton>
      <div className="relative">
        <IconButton title="Cart"><ShoppingBag size={18}/></IconButton>
        <span className="absolute -top-1 left-1/2 grid h-4 min-w-4 -translate-x-1/2 place-items-center rounded-full bg-primary px-1 text-[0.5rem] font-semibold text-primary-foreground">0</span>
      </div>
      <IconButton title="Search"><Search size={18}/></IconButton>
    </div>
  );
};
...
const CountrySelector = ({ className = "" }) => {
  ...
  const [value, setValue] = React.useState("AUS");
        ...
        <div className="absolute right-0 top-full z-50 mt-1 min-w-[12rem] overflow-hidden rounded-md border bg-popover p-1 shadow-md">
          {opts.map((o) => (
            <button
              ...
              className={`block w-full rounded-sm px-2 py-1.5 text-left text-xs font-medium uppercase tracking-wide transition ${value === o.value ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
              {o.label}
            </button>
          ))}
...
const MobileNav = ({ menu, home, socialLinks, helpfulLinks, contactInfo }) => {
  const { Menu } = window;
  const [open, setOpen] = React.useState(false);
  const [menuItem, setMenuItem] = React.useState(null);
  const [subMenu, setSubMenu] = React.useState(null);

  const closeAll = () => { setOpen(false); setMenuItem(null); setSubMenu(null); };
  ...
  return (
    <>
      <nav className="h-[3.75rem] border-b bg-background">
        <div className="flex h-full items-center gap-4 px-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            <SecondaryNav/>
            <IconButton title="Menu" onClick={() => setOpen(true)}><Menu size={20}/></IconButton>
          </div>
        </div>
      </nav>
      ...
      <MobileSheet open={open} onClose={closeAll}>
        ...
        />
        {/* Stage 2: mega menu for picked item */}
        <MegaMenuMobile
...
const MobileSheet = ({ open, onClose, children }) => (
  <>
    <div
      onClick={onClose}
    ...
    />
    <aside
      className={`fixed inset-y-0 left-0 z-[70] w-full overflow-hidden bg-background shadow-xl transition-transform sm:max-w-[27.75rem] ${open ? "translate-x-0" : "-translate-x-full"}`}
    >
      {children}
    </aside>
  </>
...
const RootMobilePanel = ({ visible, menu, onClose, onPick, helpfulLinks, socialLinks, contactInfo }) => {
  const { X, CircleUserRound, ChevronRight } = window;
  const linkStyle = "flex w-full items-center justify-between gap-2 border-b px-2 py-4 text-sm font-semibold leading-relaxed text-foreground hover:bg-transparent";
...
const MegaMenuMobile = ({ visible, menuItem, onBack, onClose, onPickSub }) => {
  const { X, ChevronLeft, ChevronRight } = window;
  if (!menuItem) return null;
  const linkStyle = "flex w-full items-center justify-between gap-2 border-b px-2 py-4 text-sm font-semibold leading-relaxed text-foreground";

  return (
    <div className={`absolute inset-0 overflow-auto bg-background px-6 pb-10 pt-5 transition-transform ${visible ? "translate-x-0" : "translate-x-full"}`}>
      <div className="flex items-center justify-between gap-5">
        <IconButton title="Close" onClick={onClose}><X size={18}/></IconButton>
      </div>
      <div className="flex items-center py-4">
        <IconButton title="Back" onClick={onBack}><ChevronLeft size={18}/></IconButton>
      ...
      <div className="space-y-6">
        ...
        {menuItem.megaMenu.featuredItems && (
          <div className="grid grid-cols-2 gap-[1.125rem]">
            {menuItem.megaMenu.featuredItems.map((it, i) => <MenuFeaturedLink key={i} {...it}/>)}
          </div>
        )}
        <a href={menuItem.href} className="flex h-11 w-full items-center justify-center rounded-md border border-border bg-background text-sm font-medium text-foreground hover:bg-muted">
          View {menuItem.label}
        </a>
      </div>
    </div>
...
const SubMenuMobile = ({ visible, subMenu, onBack, onClose }) => {
  const { X, ChevronLeft, ChevronRight } = window;
  if (!subMenu) return null;
  const linkStyle = "flex w-full items-center justify-between gap-2 border-b px-2 py-4 text-sm font-semibold leading-relaxed text-foreground";

  return (
    <div className={`absolute inset-0 overflow-auto bg-background transition-transform ${visible ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <div className="px-6 pb-10 pt-5">
        ...
        <div className="space-y-6">
          ...
          <a href={subMenu.href} className="flex h-11 w-full items-center justify-center rounded-md border border-border bg-background text-sm font-medium text-foreground hover:bg-muted">
            View {subMenu.label}
          </a>
        </div>
      </div>
...
const HelpfullLinks = ({ links }) => {
  if (!links) return null;
  return (
    <ul className="flex flex-col">
      {links.map((l, i) => (
        <li key={i}>
          <a href={l.href} className="inline-flex h-9 w-full items-center justify-start rounded-md px-1 text-xs font-semibold text-muted-foreground hover:bg-muted hover:text-foreground">
            {l.label}
          </a>
        </li>
      ))}
...
const ContactInfoList = ({ links }) => {
  if (!links) return null;
  const iconMap = { Phone: window.Phone, Send: window.Send };
  return (
    <ul className="flex flex-col">
      {links.map((l, i) => {
        ...
        return (
          <li key={i}>
            <a href={l.href} className="inline-flex h-9 w-full items-center justify-start gap-2 rounded-md px-1 text-xs font-semibold text-muted-foreground hover:bg-muted hover:text-foreground">
              {Icon && <Icon size={14}/>}
              {l.label}
            </a>
          </li>
...
Object.assign(window, { EcommerceNavbar1 });
