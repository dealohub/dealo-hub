/* FeaturedBrandsStrip — trusted partner logos under hero
   ─────────────────────────────────────────────────────
   Premium feel: grayscale logos, subtle top/bottom hairlines,
   marquee-style continuous scroll, hover restores color.
*/

const FeaturedBrandsStrip = () => {
  // Logos: using brand SVGs via simple icons CDN (clean monochrome)
  // Mix of automotive + property + tech brands to match a UAE marketplace
  const brands = [
    ...
    <section className="relative w-full border-y border-white/5 bg-background">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mb-6 flex items-center justify-between gap-6">
          ...
          <a href="#" className="hidden text-xs font-medium text-white/50 transition hover:text-white md:inline-flex items-center gap-1">
            Browse all dealers
            ...
            {loop.map((b, i) => (
              ...
              >
                <img
                  ...
                  onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement.innerHTML = `<span class='text-sm font-semibold tracking-wide text-white/70'>${b.name}</span>`; }}
                />
              </div>
            ))}
          </div>
...
window.FeaturedBrandsStrip = FeaturedBrandsStrip;
