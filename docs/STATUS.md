...
>
> Last updated: **2026-04-21 (late evening)** (Block A · Phase 6a+6b+6c AI Negotiator · Phase 4f Property sell wizard · Phase 4d-polish neighbourhood · Phase 5h generic /listings/[slug] · Phase 5i /categories browse hub · Phase 5j seller listing-management · **Phase 5k SEO infrastructure shipped**: dynamic sitemap.xml (static routes + categories + every live listing routed via the canonical resolver) + robots.txt (private surfaces blocked) + per-vertical OG/Twitter card metadata (rides + listings detail pages now expose share-friendly previews). Block C test foundation: **523 assertions** (508 unit + 15 RLS). 53 local commits pending push.)
...
| Block C — Test foundation | `src/lib/**/*.test.ts` + `supabase/tests/rls.sql` | **523 total assertions** (508 unit + 15 RLS). Vitest from scratch. 18 suites covering: listings validators (Filter A/B/C), property-wizard cross-module contract, detail-routing resolver, listing-management state machine (38 assertions), **sitemap composer (13 — vertical routing + hreflang + orphan-bug regression guard)**, properties validators + Law 74, format helpers, chat routing, landing types, AI negotiator (classifier 86 · offer-extract 26 · jitter 11 · orchestrator 15 · policy 31 · dialogue 25 · floor-leak 31 · safety 15 · OpenAI adapter 10). Full suite ~650ms. Caught 2 production bugs during authoring + 7 classifier dialect bugs on reactivation. |
