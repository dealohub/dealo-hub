...
>
> Last updated: **2026-04-21** (Phase 4b complete — `/properties/[slug]` detail page live, all 14 doctrine pillars rendered. 10 components + queries + 163 i18n keys. Earlier: Phase 4a schema, Phase 3d landing polish.)
...
| Properties | 🟢 Phase 4a + 4b shipped · 4c hub queued | **Detail page live** at `/[locale]/properties/[slug]`. All 14 doctrine pillars rendering: verification-tier badge (P1/P6), Law-74 ownership banner (P8), dynamic gallery filter pills including `diwaniya_room` category, structured diwaniya card (P14), chalet availability display (P4 — seasonal multipliers, weekend premium, min/max stay), off-plan payment plan (P13), bilingual title/description (P10), tri-state furnishing (P11), cheques count (P12), Filter C (P3). 10 components: verification-badge, ownership-banner, detail-header, detail-gallery, detail-key-info, detail-amenities, detail-description, detail-chalet-booking, detail-payment-plan, detail-purchase-panel, detail-similar, detail-mobile-actionbar. Queries: `getPropertyBySlug`, `getSimilarProperties` (2-pass tight/loose algo). 163 i18n keys AR+EN. Still queued: 4c hub (`/properties`), 4d navbar wire, 4e maps + chalet booking calendar. |
